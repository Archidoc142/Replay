...
export default function CharacterList({ characters, id_entity, charactersFromEntity, SMF }) {
    ...
    return (
        <div className="flex flex-col gap-4 items-center bg-[#16161696] pt-4 pb-8 rounded-xl border-4 border-[#5a5a5c]">
            ...
            <div className="px-4 flex flex-wrap justify-center gap-4 w-full">
                {charactersFromEntity?.map((character) => {
                    const characterIndex = characters.findIndex((char) => char.id === character)

                    return(
                        <Link href={"/character/" + character} className="flex items-center justify-between w-[48%] border-4 border-[#5a5a5c] bg-[#27242493] hover:bg-[#3b393993] p-2 rounded-lg cursor-pointer" key={character}>
                            <Image
                ...
                <form onSubmit={submit} className="relative w-[48%]">
